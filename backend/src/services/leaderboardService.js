...
async function createLeaderboardSlide({ presentationId, quizSlideId, quizSlideOrder }) {
  try {
    ...
    if (existingLeaderboard) {
      Logger.debug(`Leaderboard already exists for quiz ${quizSlideId}`);
    ...
    Logger.info(`Created leaderboard slide for quiz ${quizSlideId}`);
    
    return leaderboardSlide;
  } catch (error) {
    Logger.error('Error creating leaderboard slide', error);
    throw error;
  }
}
...
async function ensureLeaderboardForQuiz(presentationId, quizSlideId) {
  try {
    ...
    if (responseCount === 0) {
      Logger.debug(`No responses for quiz ${quizSlideId}, skipping leaderboard creation`);
      return null;
    }
    ...
    Logger.error('Error ensuring leaderboard for quiz', error);
    throw error;
  }
}
...
async function getLeaderboardForQuiz(presentationId, quizSlideId, limit = 10) {
  try {
    ...
    Logger.error('Error getting leaderboard for quiz', error);
    throw error;
  }
}
...
async function deleteLeaderboardForQuiz(quizSlideId) {
  try {
    ...
    Logger.info(`Deleted leaderboard for quiz ${quizSlideId}`);
    return result;
  } catch (error) {
    Logger.error('Error deleting leaderboard for quiz', error);
    throw error;
  }
}
...
async function createLeaderboardsForPresentation(presentationId) {
  try {
    ...
    Logger.error('Error creating leaderboards for presentation', error);
    throw error;
  }
}
